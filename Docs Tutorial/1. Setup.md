...
![第一次打开 VS Code 时的界面](/Docs%20Tutorial/Assets/setup-vscode-first-launch.png)
...
![安装中文插件](/Docs%20Tutorial/Assets/setup-vscode-chinese-extension.png)
...
![Git 下载途径示意 1](/Docs%20Tutorial/Assets/setup-git-download-source-1.png)

图 3：Git 下载途径示意图。

![Git 下载途径示意 2](/Docs%20Tutorial/Assets/setup-git-download-source-2.png)
...
安装完 Git 后，还需要配置 Git 的用户名和邮箱，否则提交时可能会报错，提示你先设置 `user.name` 和 `user.email`。
...
- 这里的用户名和邮箱，建议尽量与 GitHub 账号中使用的名字和邮箱保持一致。
- 这样后续提交记录会更容易识别，也更方便和 GitHub 账号对应起来。
- 这不是绝对强制必须完全一样，但对于新手来说，直接保持一致最省事。

---

## 5. 在 VS Code 中登录 GitHub
...
![在 VS Code 中登录 GitHub](/Docs%20Tutorial/Assets/setup-vscode-signin-github-source.png)
...
![搜索并选择 RMS-Server/RMS-Docs](/Docs%20Tutorial/Assets/setup-clone-rms-docs-search.png)

图 6：以 `RMS-Docs` 为关键词搜索，并选择 `RMS-Server/RMS-Docs`。

![服务器文档已成功拷贝到本地](/Docs%20Tutorial/Assets/setup-clone-rms-docs-local-copy.png)
