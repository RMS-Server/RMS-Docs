...
![提交修改区域](/Docs%20Tutorial/Assets/submit-commit-changes.png)
...
1. 提交后，页面会跳转。**不要点击任何按钮**，直接回到原仓库：[https://github.com/RMS-Server/RMS-Docs](https://github.com/RMS-Server/RMS-Docs)。

2. 在原仓库主页上方，你会看到一条黄色提示条，类似：
   ```
   你的分支名 had recent pushes less than a minute ago
   ```

![推送提示和 PR 按钮](/Docs%20Tutorial/Assets/push-notification-and-pr-button.png)
...
![确认 PR 方向](/Docs%20Tutorial/Assets/pr-confirm-direction.png)

图 3：确认 PR 的方向正确——从你的 Fork 向原仓库提交。

5. 填写 PR 的标题和说明。

![填写 PR 标题和说明](/Docs%20Tutorial/Assets/pr-title-description.png)
...
![同步 Fork](/Docs%20Tutorial/Assets/sync-fork.png)
...
完成第一次贡献后，后续的流程就简单多了。但有一点务必注意：

> [!IMPORTANT]
> **每次开始新的修改前，必须先同步你的 Fork，确保你的 master 分支是基于原仓库的最新提交。不要在落后的 master 或上次用过的旧分支上继续修改。**

```text
1. 回到自己 Fork 的仓库，同步 master 到原仓库最新（Sync fork → Update branch）
2. 找到文件并编辑
3. 提交时选择 "Create a new branch for this commit"，基于最新 master 创建新分支
4. 回到原仓库，创建 PR（确认 base repository 是 RMS-Server/RMS-Docs）
5. 等待审核和合并
6. 合并后再次同步 Fork，为下一次修改做准备
```
